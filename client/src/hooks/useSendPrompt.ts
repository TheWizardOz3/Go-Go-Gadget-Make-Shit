...
/**
 * Response from the local new session API
 */
interface LocalNewSessionResponse {
  success: boolean;
  pid?: number;
  message?: string;
}
...
/**
 * Options for local new session
 */
export interface LocalNewSessionOptions {
 ...
 *
 * @param sessionId - The session ID to send prompts to, or null for new sessions
 * @param defaultCloudOptions - Default options for cloud execution
 * @param localSessionOptions - Options for creating new local sessions
...
export function useSendPrompt(
  ...
  localSessionOptions?: LocalNewSessionOptions
): UseSendPromptReturn {
  ...
  /**
   * Start a new session locally with a prompt
   */
  const startNewSessionLocal = useCallback(
    async (prompt: string, projectPath: string): Promise<SendPromptResult> => {
      debugLog.info('startNewSessionLocal called', { projectPath, promptLen: prompt.length });

      setIsSending(true);
      setError(null);

      try {
        debugLog.info('startNewSessionLocal: Creating new session', { projectPath });

        const response = await api.post<LocalNewSessionResponse>('/sessions/new', {
          projectPath,
          prompt,
        });

        debugLog.info('startNewSessionLocal: Session created', {
        ...
        const result: SendPromptResult = {
          success: true,
          mode: 'local',
          // Note: sessionId is generated by Claude Code, client will poll to discover it
        };
        setLastResult(result);
        return result;
      } catch (err) {
        let errorMessage = 'Failed to start new session';
        ...
        debugLog.error('startNewSessionLocal: Failed', { errorMessage });
  ...
  const sendPromptLocal = useCallback(
    async (prompt: string): Promise<SendPromptResult> => {
      ...
      if (!sessionId) {
        ...
        if (localSessionOptions?.projectPath) {
          debugLog.info('sendPromptLocal: No session, creating new one');
          return startNewSessionLocal(prompt, localSessionOptions.projectPath);
        }

        debugLog.warn('sendPromptLocal: No session selected and no projectPath for new session');
        const result: SendPromptResult = {
          success: false,
          mode: 'local',
      ...
      try {
        ...
        debugLog.error('sendPromptLocal: Failed', { errorMessage });
    ...
    [sessionId, localSessionOptions?.projectPath, startNewSessionLocal]
  );
  ...
  const sendPromptAdvanced = useCallback(
    async (prompt: string, options?: CloudExecutionOptions): Promise<SendPromptResult> => {
      ...
      if (isSending) {
        debugLog.warn('sendPromptAdvanced: Already sending');
        return {
          success: false,
          mode: 'local',
      ...
      if (currentMode === 'cloud') {
        debugLog.info('sendPromptAdvanced: Cloud mode detected', {
          hasRepoUrl: !!cloudOptions?.repoUrl,
          hasProjectName: !!cloudOptions?.projectName,
        });
